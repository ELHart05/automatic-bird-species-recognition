...
st.set_page_config(page_title='Birds Species Classification')
st.title('Birds Species Image Classification')
st.subheader('ALLAOUA / BOUBENIA — 2SQ1, 2024/2025')
...
if img is not None:
    ...
    img_bat = np.expand_dims(img_array, 0)

    predict = model.predict(img_bat)[0]
    # numerically stable softmax
    exp_scores = np.exp(predict - np.max(predict))
    score = exp_scores / np.sum(exp_scores)

    st.write('Bird in image is **{}** with a confidence of **{:0.2f}%**'.format(
